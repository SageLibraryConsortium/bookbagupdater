...
## TestRun -- New Items Within a Branch ##  
...
(
 ...
 MAX(B.\"IDATE\") \"THEDATE\"
  FROM
(
SELECT (SELECT RECORD FROM ASSET.CALL_NUMBER WHERE ID=A.CALL_NUMBER AND RECORD>0 AND RECORD IS NOT NULL) \"REC\",CREATE_DATE::DATE \"IDATE\" FROM ASSET.COPY  A WHERE 
CIRC_LIB IN($ous)
