...
## Sage Top 100 New Items (only 2015/2016) ##  
...
AND RECORD IN(SELECT ID FROM BIBLIO.RECORD_ENTRY WHERE marc !~ \$\$<leader>.......(b|s)\$\$ AND marc ~ \$\$<controlfield tag=.008.>.......(2015|2016)\$\$))
...
CIRCULATE and NAME NOT IN ('Magazines','MAGAZINES','PERIODICALS','Periodicals','ADULT MAGAZINES','Stacks','Storage','CHILDREN''S MAGAZINES','ADULTOS ESPANOL REVISTAS')) 
...
## Scoped Top 100 New Items (only 2015/2016 ##
...
(
SELECT (SELECT RECORD FROM ASSET.CALL_NUMBER WHERE ID=A.CALL_NUMBER AND RECORD>0 AND RECORD IS NOT NULL
AND RECORD IN(SELECT ID FROM BIBLIO.RECORD_ENTRY WHERE ID IN(SELECT RECORD FROM ASSET.CALL_NUMBER WHERE OWNING_LIB IN($ous)) AND marc !~ \$\$<leader>.......[bs]))
...
('Magazines','MAGAZINES','PERIODICALS','ADULT MAGAZINES','Adult Magazines','Stacks','Storage','CHILDREN''S MAGAZINES','Young Adult Magazines','ADULTOS ESPANOL REVISTAS'))
AND OPAC_VISIBLE AND HOLDABLE AND
CIRCULATE AND ID != -1::BIGINT
ORDER BY
CREATE_DATE::DATE DESC LIMIT 300
...
## Sage Top 100 New YA Items (only 2015/2016)##
...
(
SELECT (SELECT RECORD FROM ASSET.CALL_NUMBER WHERE ID=A.CALL_NUMBER AND RECORD>0 AND RECORD IS NOT NULL
AND RECORD IN(SELECT ID FROM BIBLIO.RECORD_ENTRY WHERE marc !~ \$\$<leader>.......[bs]\$\$ AND marc ~ \$\$<controlfield tag=.008.>.......(2015|2016)\$\$))
...
## Sage Top 100 New Large Print Items (only 2015/2016)##
...
(
SELECT (SELECT RECORD FROM ASSET.CALL_NUMBER WHERE ID=A.CALL_NUMBER AND RECORD>0 AND RECORD IS NOT NULL
AND RECORD IN(SELECT ID FROM BIBLIO.RECORD_ENTRY WHERE marc ~ \$\$<controlfield tag=.008.>.......(2015|2016)\$\$))
\"REC\",CREATE_DATE::DATE FROM ASSET.COPY  A 
WHERE LOCATION IN(SELECT ID FROM ASSET.COPY_LOCATION WHERE OPAC_VISIBLE AND HOLDABLE AND 
